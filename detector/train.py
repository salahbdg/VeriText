...
def setup_distributed(port=29500):
    ...
    if not dist.is_initialized():
        if 'RANK' not in os.environ or 'WORLD_SIZE' not in os.environ:
            raise ValueError("RANK and WORLD_SIZE environment variables are required for distributed training")
...
def load_datasets(data_dir, real_dataset, fake_dataset, tokenizer, batch_size,
    ...
    if fake_dataset == 'TWO':
        download(real_dataset, 'small-117M', 'small-117M-nucleus', data_dir=data_dir)
    elif fake_dataset == 'THREE':
        download(real_dataset, 'small-117M', 'small-117M-k40', 'small-117M-nucleus', data_dir=data_dir)
    else:
    ...
    if fake_dataset == "TWO":
        ...
        fake_corpora = [Corpus(name, data_dir=data_dir) for name in ['small-117M', 'small-117M-nucleus']]
...
def train(model: nn.Module, optimizer, device: str, loader: DataLoader, desc='Train'):
    ...
    with tqdm(loader, desc=desc, disable=distributed() and dist.get_rank() > 0) as loop:
        for texts, masks, labels in loop:
            ...
            optimizer.zero_grad()
            outputs = model(texts, attention_mask=masks)
            loss = criterion(outputs.logits, labels)

            loss.backward()
            optimizer.step()

            batch_accuracy = accuracy_sum(outputs.logits, labels)
...
def run(max_epochs=None,
        ...
        fake_dataset='small-117M-nucleus',
        token_dropout=None,
        large=False,
        learning_rate=2e-5,
        weight_decay=0,
    ...
    for epoch in epoch_loop:
        ...
        # Ensure environment variables are set before validation
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fake-dataset', type=str, default='small-117M-k40')
